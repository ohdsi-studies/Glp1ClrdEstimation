...
inputValues <- data.frame("serverDatabaseId"=c("truven_ccae",
                                                "optum_extended_dod",
                                                "truven_mdcd",
                                                "optum_ehr",
                                                ...
                                                "truven_mdcr",
                                                "jmdc"),
                           "cdmDatabaseSchema"=c("cdm_truven_ccae_v1676",
                                                    "cdm_optum_extended_dod_v1679",
                                                    "cdm_truven_mdcd_v1561",
                                                    "cdm_optum_ehr_v1562",
                                                    "cdm_iqvia_pharmetrics_plus_v1500",
                                                    "cdm_truven_mdcr_v1692",
                                                    "cdm_jmdc_v1678"),
                           "cohortTable"=c("HERMES_p4v3_TruvenCCAE",
                                              "HERMES_p4v3_OptumDOD",
                                              "HERMES_p4v3_TruvenMDCD",
                                              "HERMES_p4v3_OptumEHR",
                                              "HERMES_p4v3_PharmetricsPlus",
                                              "HERMES_p4v3_TruvenMDCR",
                                              "HERMES_p4v3_JMDC"),
                           "databaseId"=c("TruvenCCAE",
                                              "OptumDOD",
                                              "TruvenMDCD",
                                              "OptumEHR",
                                              "PharmetricsPlus",
                                              "TruvenMDCR",
                                              "JMDC"),
                           "databaseName"=c("IBM Commercial Claims and Encounters (CCAE) v1676)",
                                               "Optum Clinformatics Extended Data Mart - Date of Death (DOD) v1679",
                                               "IBM MarketScan Multi-State Medicaid (MDCD) v1561",
                                               "Optum Pan-Therapeutic Electronic Health Records (OptumEHR) v1562",
                                               "IQVIA Adjudicated Health Plan Claims (PharmetricsPlus) v1500",
                                               "IBM MarketScan Medicare Supplemental (v1692)",
                                               "Japan Medical Data Center (JMDC)"))
           
cohortDatabaseSchema <- "scratch_mconove1" # change to your scratch space

for (i in 1:nrow(inputValues)) { # i <- 7

        connectionDetails <- serverDatabaseString <- cdmDatabaseSchema <- cohortTable <- databaseId <- databaseName <- databaseDescription <- NULL
        
        serverDatabaseString <- paste0("ohda-prod-1.cldcoxyrkflo.us-east-1.redshift.amazonaws.com/",inputValues$serverDatabaseId[i])
        cdmDatabaseSchema <- inputValues$cdmDatabaseSchema[i]
        cohortTable <- inputValues$cohortTable[i]
        databaseId <- inputValues$databaseId[i]
        databaseName <- inputValues$databaseName[i]
        databaseDescription <- inputValues$databaseName[i]
        ...
        #cohortTable <- "HERMES_p4v2_TruvenCCAE" #Revise cohortTable name to refer to the specific DB being run
        #cohortTable <- "HERMES_p4v2_OptumDOD" #Revise cohortTable name to refer to the specific DB being run
        #cohortTable <- "HERMES_p4v2_TruvenMDCD" #Revise cohortTable name to refer to the specific DB being run
        #cohortTable <- "HERMES_p4v2_OptumEHR" #Revise cohortTable name to refer to the specific DB being run
        #cohortTable <- "HERMES_p4v2_PharmetricsPlus" #Revise cohortTable name to refer to the specific DB being run
        #cohortTable <- "HERMES_p4v2_TruvenMDCR" #Revise cohortTable name to refer to the specific DB being run
        #cohortTable <- "HERMES_p4v2_JMDC" #Revise cohortTable name to refer to the specific DB being run
...
resultsZipFilePharmetricsPlus <- file.path("D:/StudyResults/GLP1Repro/CohortMethod/HERMESp4Hydra/PharmetricsPlus/export", paste0("Results_", "PharmetricsPlus", ".zip"))
...
preMergeShinyData <- function(shinyDataFolder) { # shinyDataFolder <- "D:/StudyResults/GLP1Repro/CohortMethod/HERMESp4Hydra/shinyData"
        shinySettings <- list(dataFolder = shinyDataFolder, blind = FALSE)
